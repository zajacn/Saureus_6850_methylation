...
final_results <- efficient_overlap(high_diff_bins, mygffdf)

#
(fn <- paste("Different_Methylation_inBins_", clone_of_interest, "_category_", COI, ".tsv", sep = ""))
write_tsv(final_results, fn)


# joining in PrX
final_results$TSBnPASN <- final_results$TSB_count + final_results$PASN_count

# Join with prX data
final_results_wPrX <- left_join(final_results, prX, by = c("locus_tag" = "LocusTag"))

(fn <- paste("Different_Methylation_inBins_wPrX_", clone_of_interest, "_category_", COI, ".tsv", sep = ""))
write_tsv(final_results_wPrX, fn)


# plot some
plot(log2(final_results_wPrX$mean_iBAQ) ~ sqrt(final_results_wPrX$TSBnPASN),
     xlab = "sqrt(TSB + PASN Methylation Count)",
     ylab = "log2 mean iBAQ",
     main = paste("Methylation Count vs mean iBAQ for", clone_of_interest, "category", COI))


myCols <- c(32:35, 39, 43, 44, 81:91)

(fN <- paste("Pairwise_Relationships_", clone_of_interest, "_category_", COI, ".png", sep = ""))
pdf(fN, width = 20, height = 20)
pairs(final_results_wPrX[, myCols],
      main = paste("Pairwise Relationships for", clone_of_interest, "category", COI),
      pch = 19,
      col = ifelse(final_results$dominant_treatment == "PASN", "blue", "red"))
dev.off()
